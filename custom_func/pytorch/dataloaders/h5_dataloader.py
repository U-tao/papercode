...
from custom_func.utils import load_h5
import torch
import logging
...
class Dataset(data.Dataset):
    ...
    def load_data_array(self, data_path):
        ...
        for col in all_cols:
            if col in self.feature_map.labels and col not in data_dict.keys():
                warnings.warn(f'{col} 不在生成的 DataLoder 中')
                continue
            else:
                array = data_dict[col]
            if array.ndim == 1:
...
class DataLoader(data.DataLoader):
    def __init__(self,
        ...
        if np.mod(self.num_samples * 1.0, self.batch_size) == 1:
            self.num_batches = int(np.floor(self.num_samples * 1.0 / self.batch_size))
        else:
...
class H5DataLoader(object):
    def __init__(self,
        ...
        if stage in ["both", "train"]:
            train_gen = DataLoader(feature_map, train_data, batch_size=batch_size, shuffle=shuffle, drop_last=True, **kwargs)
