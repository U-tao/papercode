...
from custom_func.pytorch.torch_utils import get_device, get_optimizer, get_loss, get_regularizer
from custom_func.utils import Monitor
...
class BaseModel(nn.Module):
    ...
    def compile(self, optimizer, loss, lr, custom=False):
        if custom:
            self.optimizer = get_optimizer(optimizer, self.parameters(), lr, self.kwargs)
    ...
    def get_inputs(self, inputs, feature_source=None):
        ...
        for feature, spec in self.feature_map.features.items():
            ...
            if spec["type"] == "meta":
                continue
            c = self.feature_map.get_column_index(feature)
    ...
    def checkpoint_and_earlystop(self, logs, min_delta=1e-6):
        ...
        # 判断是否停止（停止：self._stop_training = True）
    ...
    def evaluate(self, data_generator, metrics=None, is_test=False):
        ...
        with torch.no_grad():
            ...
            if metrics is not None:
                ...
                val_logs = self.evaluate_metrics(y_true, y_pred, self.validation_metrics, group_id, is_test,
                                                 self.kwargs)
            logging.info('===')
    ...
    def evaluate_metrics(self, y_true, y_pred, metrics, group_id=None, is_test=False, **params):
