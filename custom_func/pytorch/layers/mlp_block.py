...
class MLP_Block(nn.Module):
    def __init__(self, 
        ...
        elif len(dropout_rates) < len(hidden_units):
            dropout_rates = [dropout_rates[0]]* len(hidden_units)

        if not isinstance(hidden_activations, list):
            hidden_activations = [hidden_activations] * len(hidden_units)
        elif len(hidden_activations) < len(hidden_units):
            hidden_activations = [hidden_activations[0]]* len(hidden_units)
