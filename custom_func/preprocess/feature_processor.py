...
class FeatureProcessor(object):
    def __init__(self,
        ...
        self.dtype_dict = dict(
            (feat["name"], eval(feat["dtype"])
            if type(feat["dtype"]) == str
            else feat["dtype"])
        ...
        for col in all_cols:
            ...
            if name in ddf.columns:
                ...
                if col not in self.label_cols:
                    if col['type'] in ['categorical'] and ddf[name].dtype in ['object']:
                        le = LabelEncoder()
                        ddf[name] = le.fit_transform(ddf[name])
                # 类型转换 int object -> float
    ...
    def _fill_na_(self, col, series):
        ...
        if na_value is not None:
            ...
            if series.dtype in ['float']:
                return series.fillna(series.mean())
            else:
                return series.fillna(series.mode()[0])
        elif col['type'] in ['categorical']:
            if series.dtype in ['object']:
                return series.fillna('其他')
            else:
        ...
        for col in self.feature_cols:
            ...
            if col["active"]:
                ...
                if col["type"] == "meta": # e.g. group_id
                    ...
                    raise NotImplementedError("feature_col={}".format(feature_cols))
