...
def enumerate_params(config_file, exclude_expid=[]):
    ...
    if "dataset_config" in config_dict:
        ...
        # 加载 dataset_config.yaml 中的参数
        dataset_dict = load_dataset_config(base_config_dir, dataset_id)

    if model_dict["dataset_id"] in ["", "TBD"]:  # rename base expid
        model_dict["dataset_id"] = dataset_id
        experiment_id = model_dict["src"] + "_" + dataset_id
    ...
    # 在base_config_dir 目录下创建FINAL_tuner_config 目录
    config_dir = config_file.replace(".yaml", "")
    if not os.path.exists(config_dir):
        os.makedirs(config_dir)

    # enumerate dataset para combinations（参数组合）
    ...
    # dump dataset para combinations to config file（将数据集参数组合转储到配置文件）
    ...
    # enumerate src para combinations(枚举各种参数组合，放入model_param_combs 中）
    ...
    # update dataset_id into src params
    ...
    for idx, item in enumerate(itertools.product(model_param_combs.values(),
        ...
        # add a random number to avoid duplicate during debug（加入一个随机数，避免调试过程重复）
        random_str = ""
        if para_dict["debug_mode"]:
            random_str = "{:06d}".format(np.random.randint(1e6))
        # 生成一串排列随机数字字母组合-指代特定模型参数组
    ...
    # dump src para combinations to config file（将模型参数组合转储到配置文件）
...
def grid_search(config_dir,
    ...
    total_num = len(experiment_id_list)
    time_T = 0
    ...
    while len(experiment_id_list) > 0:
        ...
        if len(idle_queue) > 0:
            ...
            cmd = sys.executable + " -u {} --config {} --expid {} --gpu {}" \
                .format(script, config_dir, expid, gpu_id)

            time_now = time.perf_counter()
            run_now = total_num - len(experiment_id_list)
            usetime_now = time_now-time_start
            time_gap = time.perf_counter() - time_T

            print(f'\n'
                  f'\n****************** GradSearch ****************** '
                  f'\n****** 正在训练  : {run_now} / {total_num} 次,'
                  f'\n****** 目前耗时  : {round(usetime_now/3600, 2)} h'
                  f'\n****** 平均耗时  : {round(usetime_now/(60 * max(run_now-1, 1)), 2)} sec/次 (上次：{round(time_gap/60, 2)} sec)'
                  f'\n****** 仍需训练  : {round(usetime_now/(3600 * max(run_now-1, 1)) * (len(experiment_id_list)+1), 2)} h'
                  f'\n')
